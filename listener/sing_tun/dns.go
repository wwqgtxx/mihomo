	...
	if dnsHandler := h.ShouldHijackDns(metadata.Destination.AddrPort()); dnsHandler != nil {
		...
		for {
			...
			err = func() error {
				...
				defer cancel()
				inData := buff[:n]
				msg, err := RelayDnsPacket(ctx, dnsHandler, inData, buff)
				if err != nil {
					return err
				}
	...
	if dnsHandler := h.ShouldHijackDns(metadata.Destination.AddrPort()); dnsHandler != nil {
		...
		for {
			...
			go func() {
				...
				msg, err := RelayDnsPacket(ctx, dnsHandler, inData, writeBuff.FreeBytes())
...
func RelayDnsPacket(ctx context.Context, dnsHandle DnsHandlerType, payload []byte, target []byte) ([]byte, error) {
	msg := &D.Msg{}
