...
func (t *gvisorAdapter) DnsHijack() []string {
	dnsHijackArr := make([]string, len(t.dnsServer.udpEndpoints)+len(t.dnsServer.tcpListeners))
	for _, id := range t.dnsServer.udpEndpointIDs {
		dnsHijackArr = append(dnsHijackArr, fmt.Sprintf("udp://%s:%d", id.LocalAddress.String(), id.LocalPort))
	}
	for _, l := range t.dnsServer.tcpListeners {
		dnsHijackArr = append(dnsHijackArr, fmt.Sprintf("tcp://%s", l.Addr().String()))
	}
...
func (t *gvisorAdapter) ReCreateDNSServer(resolver *dns.Resolver, mapper *dns.ResolverEnhancer, dnsHijackArr []string) error {
	...
	for _, addr := range dnsHijackArr {
		...
		if addrType != "" && addrType != "udp" && addrType != "tcp" {
			return fmt.Errorf("unspported dns scheme:%s", addrType)
		}
		if addrType == "udp" || addrType == "" {
